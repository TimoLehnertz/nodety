    ...
    pub fn map_scope_portals<SO: TypeExprScope>(self, mapper: &mut impl FnMut(S) -> SO) -> TypeExpr<T, SO> {
        match self {
            ...
            Self::Constructor { inner, parameters } => TypeExpr::Constructor {
                inner,
                parameters: parameters.into_iter().map(|(k, v)| (k, v.map_scope_portals(mapper))).collect(),
            },
            ...
            Self::NodeSignature(sig) => TypeExpr::NodeSignature(Box::new(sig.map_scope_portals(mapper))),
            Self::PortTypes(pt) => TypeExpr::PortTypes(Box::new(pt.map_scope_portals(mapper))),
            Self::Union(a, b) => {
                TypeExpr::Union(Box::new(a.map_scope_portals(mapper)), Box::new(b.map_scope_portals(mapper)))
            }
            ...
            Self::Intersection(a, b) => {
                TypeExpr::Intersection(Box::new(a.map_scope_portals(mapper)), Box::new(b.map_scope_portals(mapper)))
            }
            ...
            Self::ScopePortal { expr, scope } => {
                TypeExpr::ScopePortal { expr: Box::new(expr.map_scope_portals(mapper)), scope: mapper(scope) }
            }
        }
    }
}
...
impl<T: Type, S: TypeExprScope> PortTypes<T, S> {
    pub(crate) fn map_scope_portals<SO: TypeExprScope>(self, mapper: &mut impl FnMut(S) -> SO) -> PortTypes<T, SO> {
        PortTypes {
            ports: self.ports.into_iter().map(|p| p.map_scope_portals(mapper)).collect(),
...
impl<T: Type, S: TypeExprScope> TypeParameter<T, S> {
    fn map_scope_portals<SO: TypeExprScope>(self, mapper: &mut impl FnMut(S) -> SO) -> TypeParameter<T, SO> {
        TypeParameter {
            ...
            default: self.default.map(|default| default.map_scope_portals(mapper)),
        }
    }
}
...
impl<T: Type, S: TypeExprScope> NodeSignature<T, S> {
    pub(crate) fn map_scope_portals<SO: TypeExprScope>(self, mapper: &mut impl FnMut(S) -> SO) -> NodeSignature<T, SO> {
        NodeSignature {
            parameters: self.parameters.into_iter().map(|(k, param)| (k, param.map_scope_portals(mapper))).collect(),
